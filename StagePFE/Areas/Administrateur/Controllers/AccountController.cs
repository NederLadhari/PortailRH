...
namespace StagePFE.Areas.Administrateur.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public DateTime DateAjoutSuivInit(Employe employe)
        {
            try
            {
                var DateRecrutement = employe.DateRecrutement;
                int jourIncrementation = DateRecrutement.Value.Day;
                int moisIncrementation = DateTime.Now.Month;
                int anneeIncrementation = DateTime.Now.Year;
                DateTime dateAjoutSuivant = new DateTime(anneeIncrementation, moisIncrementation, jourIncrementation);
                int Resultat = DateTime.Compare(DateTime.Now, dateAjoutSuivant);
                if(Resultat>0)
                {
                    int moisSuivant = dateAjoutSuivant.Month + 1;
                    dateAjoutSuivant = new DateTime(anneeIncrementation, moisSuivant, jourIncrementation);
                    return dateAjoutSuivant;
                }
                return dateAjoutSuivant;
        ...
        [AllowAnonymous]
        //[ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(String firstname, String secondname, String email, String passe, String sexe, String confirm,String jobb, String telephone, String naissance, String recrutement, String adresse, String cin)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await UserManager.CreateAsync(user, passe);
                     
                if (result.Succeeded)
                {
                        var result1 = UserManager.AddToRole(user.Id, jobb);
                    ...
                    var e = new Employe
                    {
                        Nom = firstname,
                        Prenom = secondname,
                        DateNaissance= DateTime.Parse(naissance, MyCultureInfo),
                        DateRecrutement =DateTime.Parse(recrutement, MyCultureInfo),
                        //Sexe = model.Sexe,
                        // Poste = model.Poste,
                        Poste=jobb,
                        Sexe=sexe,
                    ...
                    var c = new Compte
                    {
                        ...
                        Solde = 0,
                        Credit=0,
                        DateAjoutSuivante = DateAjoutSuivInit(e)

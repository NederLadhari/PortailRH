...
namespace StagePFE.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public bool MiseAJourSolde(Employe employe, Compte compte)
        {
            try
            {
                bool maj = false;
                var DateRecrutement = employe.DateRecrutement;
                DateTime DateAjout = (DateTime)compte.DateAjoutSuivante;
                if (DateTime.Now >= DateAjout)
                {
                    if (DateTime.Now > DateAjout)
                    {
                        var diff = (DateTime.Now - DateAjout).Days;
                        int nbMois = diff / 30;
                        if (nbMois >= 2)
                        {
                            var NouvelleDate = DateAjout.AddMonths(nbMois);
                            compte.Solde += 1.75 * nbMois;
                            compte.DateAjoutSuivante = NouvelleDate;
                            if (compte.DateAjoutSuivante <= DateTime.Now)
                            {
                                int MoisSuivant = DateAjout.Month + 1;
                                compte.DateAjoutSuivante = new DateTime(DateTime.Now.Year, MoisSuivant, DateAjout.Day);
                                compte.Solde += 1.75;
                            }
                            maj = true;
                        }
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    var employe = db.Employe
                       .Where(u => u.IdUser == users.Id)
                       .FirstOrDefault();
                    var compte = db.Compte
                    ...
                    if (role.Contains("Administrateur"))
                    {
                        //send him to Admin controller and index action
                        return RedirectToAction("DashboardAdmin", "Dashboard", new { area = "Administrateur" });
                    }
                    else if (role.Contains("Chef de projet"))
                    {
                        //send him to ChefProjet controller and index action
                        var resultat = MiseAJourSolde(employe, compte);
                        return RedirectToAction("DashboardChefProjet", "DashboardChef", new { area = "ChefProjet" });
                    }
                    else if (role.Contains("RH"))
                    {
                        //send him to Admin controller and index action
                        return RedirectToAction("DemandeRH", "DemandeRH", new { area = "RH" });
                    }
                    else
                    {
                        //send him to Ingenieur controller and index action

                        //var users = UserManager.FindById(User.Identity.GetUserId());
                        //var recrutement = db.Employe
                        //      .Where(u => u.IdUser == users.Id)
                        //      .FirstOrDefault().DateRecrutement;
                        //DateTime rec = (DateTime)recrutement;
                        //var difference = recrutement - DateTime.Now.Date;
                        //int mois = NombreDeMois(rec, DateTime.Today);
                        var resultat = MiseAJourSolde(employe, compte);
                        return RedirectToAction("Accueil", "Home", new { area = "Ingenieur" });
                    }
                    

                    //var dEmploye = db.Employe
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var e = new Employe
                    {
                        ...
                        DateRecrutement = model.Recrutement,
                        Sexe = model.Sexe,
                        Poste = model.Poste,
